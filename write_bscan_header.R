...
write_bscan_header <- function(vol_con, bscan_header_list) {
    ...
    writeBin(bscan_header_list$bscan_hdr_size, vol_con, integer())
    writeBin(bscan_header_list$start_x, vol_con, double())
    writeBin(bscan_header_list$start_y, vol_con, double())
    writeBin(bscan_header_list$end_x, vol_con, double())
    writeBin(bscan_header_list$end_y, vol_con, double())
    writeBin(bscan_header_list$num_seg, vol_con, integer())
    writeBin(bscan_header_list$off_seg, vol_con, integer())
    writeBin(bscan_header_list$quality, vol_con, what="numeric", size = 4)
    # bscan_header_list$spare %>%
    #     charToRaw() %>%
    #     pad_raw(n = 196) %>%
    #     writeBin(vol_con, "raw")
    writeBin(raw(196), vol_con, "raw")
